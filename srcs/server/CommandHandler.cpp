...
/*   Updated: 2025/07/29 13:43:24 by cschmid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 				// chan->changeModeO(*client, std::string(1, sign)
//					+ "o",*targetClient);
// 			}
// 			else
...
void Server::handleMODE(Client *client, const Message &msg)
{
	...
	for (size_t i = 0; i < modes.size(); ++i)
	{
		...
		handleModeAction(client, chan, msg, c, sign);
	}
}

void Server::handleModeAction(Client *client, Channel *chan, const Message &msg,
	char c, char sign)
{
	switch (c)
	{
		case 'i':
			handleModeI(client, chan, sign);
			break ;
		case 't':
			handleModeT(client, chan, sign);
			break ;
		case 'k':
			handleModeK(client, chan, msg, sign);
			break ;
		case 'o':
			handleModeO(client, chan, msg, sign);
			break ;
		case 'l':
			handleModeL(client, chan, msg, sign);
			break ;
		default:
			std::cout << "Mode inconnu : " << c << std::endl;
			break ;
...
void Server::handleModeI(Client *client, Channel *chan, char sign)
{
	(void)chan;
	(void)sign;
	(void)client;
...
void Server::handleModeT(Client *client, Channel *chan, char sign)
{
	(void)chan;
	(void)sign;
	(void)client;
...
void Server::handleModeK(Client *client, Channel *chan, const Message &msg,
	char sign)
{
	(void)chan;
	(void)sign;
...
void Server::handleModeO(Client *client, Channel *chan, const Message &msg,
	char sign)
{
	Client	*targetClient;
...
void Server::handleModeL(Client *client, Channel *chan, const Message &msg,
	char sign)
{
	int	limit;

...
class Server
{
  ...
  public:
	...
	void handleClientRead(Client *client,
							const std::string &input); // Handle full commands
	void handleClientDisconnection(Client *client, int client_fd,
		ssize_t received_bytes);
	// Manage disconnect or error
	...
	bool PrefixUser(const Message &msg, std::string &User, std::string &channel,
		std::string &key);
	std::string userPrefix(const std::string &prefix);
	Channel *findChannel(const std::string &name);
	Client *findClient(const std::string &nickname);
	std::vector<std::string> splitComma(const std::string &input);
	void handleSingleJoin(Client *client, const std::string &channelName,
	...
	/*===== Utilities =====*/
	void sendError(int fd, const std::string &code, const std::string &target,
					const std::string &message); // IRC error sender
